...
const PharmacyProfileScreen: React.FC = () => {
  ...
  const mapAvailable = Boolean(NativeMapView && NativeMarker);

  const openLocationPicker = () => {
    if (!mapAvailable) {
      Alert.alert(t('pharmacies.mapNotSupported'));
      return;
    }
  ...
  useEffect(() => {
    if (!mapAvailable || !mapVisible || !mapCoordinate) {
      return;
    }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.form}>
        ...
        <Button mode="outlined" icon="map-marker" onPress={openLocationPicker} style={styles.mapButton} disabled={!mapAvailable}>
          {t('pharmacies.selectOnMap')}
        </Button>
        <HelperText type={mapAvailable ? 'info' : 'error'}>
          {t(mapAvailable ? 'pharmacies.mapInstructions' : 'pharmacies.mapNotSupported')}
        </HelperText>
        <TextInput
          label={t('pharmacies.latitude')}
          value={latitude}
          style={styles.input}
          editable={!mapAvailable}
          ...
          <Modal visible={mapVisible} onDismiss={() => setMapVisible(false)} contentContainerStyle={styles.mapModal}>
            ...
            <View style={styles.mapWrapper}>
              {NativeMapView ? (
                ...
                >
                  {mapCoordinate && NativeMarker ? (
                    <NativeMarker coordinate={mapCoordinate} draggable onDragEnd={handleMarkerDragEnd} />
                  ) : null}
                </NativeMapView>
              ) : null}
            </View>
